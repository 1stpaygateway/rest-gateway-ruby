...
class ServiceClient
	...
	def performRequest(apiRequest, callBackSuccess, callBackFailure)
		...
		if (response.has_key? "isError" and response["isError"]==true)
            ...
            @result["validations"] = response["validationFailures"]
			...
			@status = "Success"
			if (response["data"].is_a?(Hash) and response["data"].has_key? "isPartial" and response["data"]["isPartial"]==true)
				@result["partialOrder"] = {"partialId" => response["data"]["partialId"], 
				"amountRemaining" =>(response["data"]["originalFullAmount"] - response["data"]["partialAmountApproved"]),
				"originalFullAmount" => response["data"]["originalFullAmount"],
				"amountApproved" => response["data"]["partialAmountApproved"],
			...
			puts ("Did not receive proper response. Check input and try again.")
		end
	end
